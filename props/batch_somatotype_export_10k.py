"""
batch_somatotype_export_10k.py — dataset_10k icin somatotip gorsel dogrulama probe'u.
...
DATASET_CSV = r"C:\Users\aliya\workspace\cc5-scripts\dataset_10k.csv"
OUTPUT_DIR  = r"C:\Users\aliya\workspace\cc5-scripts\fbx_export"
LOG_DIR     = r"C:\Users\aliya\workspace\cc5-scripts\logs"
PROBE_CSV   = os.path.join(LOG_DIR, "somatotype_probe_10k.csv")
OVERWRITE   = True
N_PER_SOMA  = 10
...
# ── Veri yukle ve oranlar hesapla ─────────────────────────────────────────────
rows = []
with open(DATASET_CSV, encoding="utf-8") as f:
    for r in csv.DictReader(f):
        w = float(r["waist_circ_pred_cm"])
        h = float(r["hip_circ_pred_cm"])
        c = float(r["chest_circ_pred_cm"])
        r["_wst_hip"]   = str(w / h) if h else "1.0"
        r["_hip_chest"] = str(h / c) if c else "1.0"
        rows.append(r)

dataset = {r["char_id"]: r for r in rows}

# ── Somatotip tipiklik skoru ──────────────────────────────────────────────────
...
def select_diverse(soma, n):
    candidates = [r for r in rows if r.get("somatotype") == soma]
    if not candidates:
        return []
    ...
    # Eksik kalirsa greedy max-min mesafe
    if len(chosen) < n:
