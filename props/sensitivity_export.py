"""
sensitivity_export.py — CC5 Script Editor'da çalıştır.
Her satır için: tüm morphları 0 yap, sadece belirtilen morphu set et, FBX export et.

compute_all_weights KULLANILMAZ — direkt CC5 morph ID'leri test edilir.
"""
...
OUTPUT_DIR = r"C:\Users\aliya\workspace\cc5-scripts\fbx_export_sensitivity"
LOG_PATH   = r"C:\Users\aliya\workspace\cc5-scripts\logs\sensitivity_export.log"
OVERWRITE  = False
...
log(f"=== sensitivity_export START {time.strftime('%Y-%m-%d %H:%M:%S')} ===")
log(f"Toplam: {total} probe")

for i, row in enumerate(rows):
    char_id    = row["char_id"]
    gender     = row["gender"]
    morph_id   = row["morph_id"]       # boş string → baseline
