...
8. [Somatotip Dağılımı](#8-somatotip-dagilimi)
9. [Açık Konular](#9-acik-konular)
...
| `hip_score` | float [0.2,0.9] | Kalça genişliği — fat'tan bağımsız kemik yapısı parametresi |
| `waist_def_score` | float [0.2,0.9] | Bel tanımı — yüksek = dar bel, düşük = geniş bel |
| `somatotype` | kategori | Proxy çevre kurallarından türetilmiş etiket (5 değer) |
...
## 8. Somatotip Dağılımı

Somatotip kolonu fat/muscle/hip_score/waist_def_score'dan proxy çevre modeli ile deterministik olarak türetilmiştir — rastgele değil.
...
**Gözlemler:**
- Rectangle baskın çünkü kalan (else) kategorisi.
- Hourglass nadir: göğüs ≈ kalça (<%3 fark) + bel her ikisinden %30+ dar — üç koşulun aynı anda sağlanması düşük olasılıklı.
- Cinsiyet dağılımı neredeyse simetrik (fat-bağımsız hip_score/waist_def_score cinsiyete göre farklılaşmıyor).
- Pear ve apple fat aralığıyla zıt: pear düşük-orta fat'ta, apple yüksek fat'ta baskın.

### Proxy Sınıflandırma Kuralları
