...
silhouette  [1, 512, 512]
     │
  concat(dim=0)
     │
  [4, 512, 512]   ←── backbone'a girecek
```
...
[4, 512, 512]
     │
  Stage 1  →  feat_256  [24,  256, 256]
  Stage 2  →  feat_128  [32,  128, 128]   ←── skip connection (seg head için saklanır)
  Stage 3  →  feat_64   [48,   64,  64]   ←── skip connection (seg head için saklanır)
  Stage 4  →  feat_32   [136,  32,  32]
  Stage 5  →  F         [384,  16,  16]   ←── ana feature map
```
...
`F` her şeyin çıkış noktası. 16×16 spatial grid, her hücre yaklaşık 32×32 piksellik bir bölgeyi temsil ediyor. 384 kanal o bölgenin ne olduğunu encode ediyor.
...
F  [384, 16, 16]
  ...
  upsample(×2)  →  [256, 32, 32]
     │
  concat(feat_64 [48, 64, 64] → 1×1 Conv → [128, 64, 64])   ←── skip
  (önce upsample×2 → [256, 64, 64], sonra cat)
     │
  Conv  →  [256, 64, 64]
     │
  upsample(×2)  →  [256, 128, 128]
     │
  concat(feat_128 [32, 128, 128] → 1×1 Conv → [64, 128, 128])   ←── skip
     │
  Conv  →  [128, 128, 128]
     │
  Conv 1×1  →  [K, 128, 128]   ←── K = bölge sayısı (~14-15)
     │
...
seg_pred  [K, 128, 128]
     │
  interpolate(size=16×16)   ←── F ile aynı çözünürlüğe indir
     │
  seg_weights  [K, 16, 16]
...
Her bölge k için:
  weighted_feat_k = sum(F × seg_weights[k]) / sum(seg_weights[k])
                  = [384]    ←── o bölgenin özelliği

  Örnek:
    k=arm    →  weighted_feat_arm   [384]   (kol bölgesinin feature'ı)
    k=torso  →  weighted_feat_torso [384]   (gövdenin feature'ı)
    ...

global_feat = mean(F, dim=(H,W))  =  [384]   (tüm vücudun genel feature'ı)

view_feature = concat([region_0, region_1, ..., region_K-1, global_feat])
             = [(K+1) × 384]
             → Linear(384 → 128) per region
             = [(K+1) × 128]
```
...
```
Görüş 0 (front)        →  view_feature_0  [(K+1)*128]
Görüş 1 (back)         →  view_feature_1  [(K+1)*128]
...
Görüş 7 (back_right)   →  view_feature_7  [(K+1)*128]

     │
  Linear((K+1)*128 → 512)   (shared weights, tüm görüşler için aynı katman)
     │
  [8, 512]
...
L_seg  =  CrossEntropy(seg_pred [K, 128, 128], gt_mask [128, 128])
